...
class PrivateMatchmakingSocket {
  ...
  onopen() {
    console.log("Connected to the Private Matchmaking websocket");
  ...
  onclose() {
    console.log("Disconnected from the Private Matchmaking websocket");
  }

  open() {
    if (!this.socket || this.socket.readyState === WebSocket.CLOSED) {
      this.socket = new WebSocket(config.websocketurl + "/ws/pong/privatematchmaking/" + match_name + "/");
...
export async function initComponent() {
  ...
  if (!match_name)
  {
    customalert("Error", "Match name not found", true);
    router.navigate('/');
  }
